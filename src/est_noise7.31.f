...
c  Intermediate covariance and filter functions; double precision
     ...
     &     ' including'
       ...
       print*,' Version 7.31  --- Apr. 2023'
       print*,' '
       print*,'  The number of processors = ', omp_get_num_procs ( )
       print*,'  The number of threads    = ', omp_get_max_threads ( )
c       print*,'  The number of blas threads=',
c     &      omp_get_openblas_num_threads()
       print*,' '
...
c     whiten model
      !$omp parallel &
      !$omp shared (din, Ax,dout,Aw,Awt,ic,Finv,max_time,mdmax)
      !$omp private (i,j)
      !$omp do
        do j=1,nmod
c        do 71 j=1,nmod
...
c  construct inverse
      ...
      !$omp parallel &
      !$omp shared (nmod,ie,a2,evd,ai)
      !$omp private (i,j,k,at)
c        do 223 i=1,nmod
        do i=1,nmod
        do 224 j=1,nmod
          at=0.0
...
c
     ...
     &   0.0d+0,ainv,maxnmod)
      ...
      !$omp parallel &
      !$omp private (i,j)
      !$omp shared (nmod,ic,x,e,ainv,dw)
      !$omp do
c        do 430 i=1,nmod
        do i=1,nmod
...
c
        !$omp private (i,j)
        !$omp shared (ic,nmod,dr,ax,x,din)
        !$omp do
c        do 335 i=1,ic
        do i=1,ic
        dr(i)=0.
...
c
      ...
      chi2=0.
      !$omp private (i,j,resA)
      !$omp shared (ic,dr,covinv,chi2)
      !$omp do
      do i=1,ic
c      do 3 i=1,ic
      resA=0.
