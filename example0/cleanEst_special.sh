...
then
   ...
   echo " Usage:  cleanEst.sh -d data -f format_type [-B start_time -E end_time -O off_file -E edit_file -T exp_file -S sine_period_file -n noiseType -v -s]"
   echo "   -v  does not estimate rate"
   echo "   -s  does not estimate seasonal sinusoids"
...
progs=/Users/john/proglib/est_noise20170721/bin
...
while getopts d:f:E:O:T:n:vs option 
do
...
#   Storte the noise model type for future use
rm -f modtype.dat
