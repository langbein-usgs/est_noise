...
#     AT THIS TIME (Dec 2015), plotting is done if data formated using 'otr' or 'doy'
#
...
then
   ...
   echo "   -s Do NOT calculate annual and semi-annual periodicities"
...
while getopts d:f:E:O:T:n:i:M:S:s option 
do
...
EOF
  ...
  echo "annual and semi-annual periods not calculated"
fi
...
time "$progs"/est_noise8 < est1.in > est1.out
...
noff=`head -1 off.in | awk '{print $1}'`
n=1
while [ "$n" -le "$noff" ]
do
   ln=`expr $n + 1`
   toff=`sed -n ''$ln'p' off.in`
   if [ "$netd" = "otr"  -o "$nett" = "doy" ]
   then
...
nbust=`echo 90 $tsamDay | awk '{print int($1*$2)}'`
"$progs"/bust8 <<EOF
$netd
data.in
data.cl
$tsamDay
$nbust     #  90     #   running median filter time
